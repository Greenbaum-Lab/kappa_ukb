...
def cupy_norm_cdf(x):
	"""
	Computes the cumulative distribution function (CDF) of the standard normal distribution in CuPy.
	"""
	return 0.5 * (1 + cp.erf(x / cp.sqrt(2)))

def infer_rbar_cupy(dpgs, phenotypic_difference, r_bar_range, r_bar_preset=[], kappa_bins=51):
	"""
	CuPy-only simplified implementation of infer_rbar, optimized for bootstrapping r_bar.
	Only returns the optimal inferred r_bar.
	"""
	dpgs, phenotypic_difference = cp.asarray(dpgs), cp.asarray(phenotypic_difference)
	dpgs_norm = cp.abs(dpgs / cp.std(dpgs))

	bins = cp.linspace(0, 1, kappa_bins)
	expected = cupy_norm_cdf(bins / (1 - bins))

	r_bar_diff = []
	r_bar_values = cp.asarray(r_bar_preset) if len(r_bar_preset) > 0 else cp.arange(*r_bar_range, 0.01)

	for r_bar in r_bar_values:
		kappa = r_bar * dpgs_norm / (r_bar * dpgs_norm + cp.sqrt(1 - r_bar ** 2))
		bin_indices = cp.digitize(kappa, bins) - 1
		valid_bins = bin_indices < (len(bins) - 1)
		bin_indices = bin_indices[valid_bins]

		n_non_zero = cp.bincount(bin_indices, weights=(phenotypic_difference[valid_bins] != 0).astype(cp.int32), minlength=len(bins) - 1)
		n_positive = cp.bincount(bin_indices, weights=(phenotypic_difference[valid_bins] > 0).astype(cp.int32), minlength=len(bins) - 1)

		P = cp.where(n_non_zero > 0, n_positive / n_non_zero, 1)
		diff = cp.sum((P - expected[:-1]) ** 2 * n_non_zero)

		r_bar_diff.append(diff.item())

	min_diff_i = cp.argmin(cp.asarray(r_bar_diff))
	return r_bar_values[min_diff_i].item()

def bootstrap_rbar(params, n_iterations=1000):
	try:
		r_bar_range = cp.array(params['r_bar_range'].split(','), dtype=cp.float32)
		r_bar_preset = [] if params['r_bar_preset'] == '' else cp.array(params['r_bar_preset'].split(','), dtype=cp.float32)
	...
	dpgs, phenotypic_difference, _ = generate_pair_comparisons(
		pgs, sample_ids, phenotype_values, params['first_subset_size'], params['unique_samples']
	)

	# Convert to CuPy arrays once
	dpgs_cp = cp.asarray(dpgs)
	phenotypic_difference_cp = cp.asarray(phenotypic_difference)

	r_bar_bootstrap = []

	n_samples = dpgs_cp.shape[0]

	for _ in range(n_iterations):
		# Sample with replacement
		indices = cp.random.choice(n_samples, size=n_samples, replace=True)

		# Infer r_bar for the bootstrapped sample
		r_bar_estimate = infer_rbar_cupy(
			dpgs_cp[indices], 
			phenotypic_difference_cp[indices],
			r_bar_range,
			r_bar_preset
		)

		r_bar_bootstrap.append(r_bar_estimate)

	return cp.asnumpy(r_bar_bootstrap)
...
def get_rbar_data(params):
	...
	save_results(params, r_bar, kappa_data)
...
def run(params):
	if params['mode'] == 'bootstrap_rbar':
		return bootstrap_rbar(params)
	else
		return get_rbar_data(params)

def setup_parser():
	parser = argparse.ArgumentParser(description='Infer kappa from PRSice data')
	parser.add_argument('--mode', required=False, default='rbar_data')
